...
object RatingRepository {
    suspend fun save(rating: FilmRatingModel) {
        ...
        film?.let { filmEntity ->
            user?.let { userEntity ->
                ServiceLocator.getDbInstance().getRatingDao()
                    .save(FilmRatingEntity(0, filmEntity.filmId, userEntity.userId, rating.rating))
            }
        }
    }
    ...
    suspend fun getFilmRating(filmModel: FilmModel, email: String): Double? {
        ...
        if (film != null && user != null) {
            val rating = ServiceLocator.getDbInstance().getRatingDao()
                .getFilmRating(film.filmId, user.userId)
            if (rating != null) {
    ...
    suspend fun getAllFilmRating(film: FilmModel): Double? {
        ...
        filmEntity?.let {
            val rating =
                ServiceLocator.getDbInstance().getRatingDao().getAllFilmRating(filmEntity.filmId)
